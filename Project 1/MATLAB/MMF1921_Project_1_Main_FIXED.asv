...
%--------------------------------------------------------------------------
    % Dynamically select lambda and K via 5-fold cross-validation
...
for t = 1 : NoPeriods
    ...
    % Dynamically select lambda and K via 5-fold cross-validation
    lambda = chooseLambda(periodReturns, periodFactRet, 0:0.05:5, 5);
    K      = chooseK(periodReturns, periodFactRet, 1:7, 5);
    ...
    for i = 1 : NoModels
        ...
        T = size(periodReturns, 1);
        p = size(periodFactRet, 2);
        y_hat = repmat(mu{i}', T, 1);
        ss_res = sum((periodReturns - y_hat).^2);
        ss_tot = sum((periodReturns - mean(periodReturns)).^2);
        R2 = 1 - ss_res ./ ss_tot;
        adjR2(t, i) = mean(1 - (1 - R2) .* (T - 1) ./ (T - p - 1), 'omitnan');
...
% Calculate average adjusted R² across the 5 rebalancing periods
meanAdjR2 = mean(adjR2, 1, 'omitnan');

% Display results in a table
adjR2Table = array2table(meanAdjR2, 'VariableNames', tags);
disp('Average Adjusted R^2 (In-Sample) for Each Model:');
disp(adjR2Table);
...
% Convert weekly portfolio values into weekly returns
weeklyReturns = diff(portfValue) ./ portfValue(1:end-1, :);

% Assume 52 weeks per year for annualization
weeksPerYear = 12;

% Compute annualized metrics
annReturn = (1 + mean(weeklyReturns)).^weeksPerYear - 1;
annStd    = std(weeklyReturns) * sqrt(weeksPerYear);
annSharpe = annReturn ./ annStd;

% Display as a table
perfTable = table(annReturn', annStd', annSharpe', 'VariableNames', ...
    {'AnnReturn', 'AnnStdDev', 'AnnSharpeRatio'}, 'RowNames', tags);
disp('Annualized Out-of-Sample Portfolio Performance Metrics:');
disp(perfTable);
